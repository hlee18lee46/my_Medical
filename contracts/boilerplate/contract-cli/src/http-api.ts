...
import type { CounterProviders, DeployedCounterContract } from "./common-types.js";
...
async function buildWalletWithSeed(seed: string, filename: string) {
  // contractConfig already has the full Config shape
  return await buildWalletAndWaitForFunds(contractConfig as any, seed, filename);
}

async function getProviders(seed: string): Promise<CounterProviders> {
  const wallet = await buildWalletWithSeed(seed, "medical-http-wallet.json");
  return await configureProviders(wallet as any, contractConfig as any);
}
 ...
 */
export async function updateMedicalRecordOnChain(
  patient_hash: bigint,
  encrypted_hash: bigint
): Promise<{
  txId: string;
  blockHeight: bigint;
  contractAddress: string;
  ...
  const result = await (contract.callTx as any).update_record(patient_hash, encrypted_hash);
