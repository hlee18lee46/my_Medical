...
export class DynamicCLIGenerator {
    ...
    async initialize() {
        try {
            ...
            this.logger.error('Failed to analyze contract:', undefined, error);
            throw error;
        }
    }
    /**
     ...
     */
    createFunctionHandler(func) {
        return async (providers, contract, rli) => {
            try {
     ...
     */
    createStateDisplayHandler() {
        return async (providers, contract) => {
            if (!this.contractAnalysis)
                return;
                                        ...
                                        if (isEmpty) {
                                            ...
                                            try {
                                                for (const item of itemsSet) {
                                                    ...
                                                    if (items.length >= 10)
                                                        break; // Limit to 10 items for display
                                                }
     ...
     */
    async collectParameter(param, rli) {
     ...
     */
    async collectSpecialParameter(param, rli) {
        if (param.name.includes('index')) {
            // For vote_for and get_vote_count functions
            const choice = await rli.question('Select option:\n  0. Option A\n  1. Option B\nEnter choice (0 or 1): ');
     ...
     */
    async executeReadOnlyFunction(functionName, args, providers, contract) {
        const api = await import('./api.js');
     ...
     */
    async executeStateChangingFunction(functionName, args, contract) {
        // Use dynamic property access to call the function
        const contractFunction = contract.callTx[functionName];
        if (!contractFunction) {
...
//# sourceMappingURL=dynamic-cli-generator.js.map